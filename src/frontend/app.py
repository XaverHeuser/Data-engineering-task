...
if st.button('Start ingestion'):
    if folder_path.strip():
        with st.spinner('Contacting backend...'):
            ...
            response = requests.post(f"{API_URL}/start-ingestion?folder_path={folder_path}")
...
if st.button('Get max recommendation'):
    ...
    if response.status_code == 200:
        result = response.json()
        st.json(result)
        st.write(f'product_id: {result.get("product_id")}, date: {result.get("date")}, Price Adjustment (%): {result.get("price_adjustment_pct")}')
